...
// Icon path data lives in icons/*.svg (and icons/sprite.svg) in the repo.
// These constants are just <use href="..."> wrappers that the cards embed —
// they don't carry SVG path data, only the reference to the symbol that the
// host page's sprite provides.
const ICON_HEART = `<span class="icon icon-heart-fill"><svg xmlns="http://www.w3.org/2000/svg" viewbox="0 0 20 20" aria-hidden="true"><use href="#icon-heart-fill"></use></svg></span>`;
const ICON_PIN = `<span class="icon icon-pin"><svg xmlns="http://www.w3.org/2000/svg" viewbox="0 0 100 100" aria-hidden="true"><use href="#icon-pin"></use></svg></span>`;
const ICON_STAR = `<span class="icon icon-star-fill"><svg xmlns="http://www.w3.org/2000/svg" viewbox="0 0 20 20" aria-hidden="true"><use href="#icon-star-fill"></use></svg></span>`;

const CARDS_TEMPLATE = `<div class="accommodation-teaser-wrapper"><div class="accommodation-teaser block" data-block-name="accommodation-teaser" data-block-status="initialized"><h2>{{Title}}</h2>{{#HasHotels}}<ul class="cards">{{#Hotels}}<li class="card" data-accommodation-id="{{Id}}"><div class="cards-card-image"><picture>{{#PictureSources}}<source media="{{media}}" srcset="{{srcset}}">{{/PictureSources}}<img loading="lazy" alt="" src="{{ImageDefault}}"></picture><button class="shortlist-btn" type="button">${ICON_HEART}<span class="loader-spinner"></span><span class="sr-text" aria-live="polite">Save {{Name}} to shortlist</span></button>{{#HasBrand}}<span class="cards-card-badge {{BrandClass}}">{{Brand}}</span>{{/HasBrand}}</div><div class="cards-card-body"><div class="cards-card-body-header"><h3><a href="{{Url}}" title="{{Name}}">{{Name}}</a></h3><button class="card-map-btn" aria-label="View {{Name}} on the map">${ICON_PIN}{{Resort}}, {{Area}}</button><div class="accommodation-ratings"><div class="accommodation-star-rating"><span class="star-ratings">{{#StarsFull}}${ICON_STAR}{{/StarsFull}}{{#StarsPlus}}<span class="star-ratings-plus" aria-hidden="true">plus</span>{{/StarsPlus}}<span class="sr-text">{{StarRating}} stars</span></span><p>Our rating</p></div>{{#HasReviews}}<div class="accommodation-rating"><img src="{{RatingImageUrl}}" alt="TripAdvisor rating: {{RatingValue}}"><p>{{NumberOfReviews}} reviews</p></div>{{/HasReviews}}</div></div>{{#HasKeySellingPoints}}<div class="cards-card-body-footer"><ul class="accommodation-features">{{#KeySellingPoints}}<li><p>{{.}}</p></li>{{/KeySellingPoints}}</ul></div>{{/HasKeySellingPoints}}</div></li>{{/Hotels}}</ul><a href="{{ShowAllLink}}" class="button">Show all ({{HotelCountForSelectedFilters}}) and filter options</a>{{/HasHotels}}{{^HasHotels}}<p class="accommodation-teaser-empty">No hotels match the selected filters.</p>{{/HasHotels}}</div></div>`;
